...
func (system *System) Debug() {

	if system.Name != "" {

		if system.Datetime.IsValid() == false {
			console.Error("system/" + system.Name + ": Invalid Datetime")
			console.Error(utils_encoding.ToJSON(system.Datetime))
		}

		if system.Distribution.IsValid() == false {
			console.Error("system/" + system.Name + ": Invalid Distribution")
			console.Error(utils_encoding.ToJSON(system.Distribution))
		}

		if system.BIOS.IsValid() == false {
			console.Error("system/" + system.Name + ": Invalid BIOS")
			console.Error(utils_encoding.ToJSON(system.BIOS))
		}

		if system.Board.IsValid() == false {
			console.Error("system/" + system.Name + ": Invalid Board \"" + system.Board.Name + "\"")
			console.Error(utils_encoding.ToJSON(system.Board))
		}

		for d := 0; d < len(system.Devices); d++ {

			var hardware = system.Devices[d]

			if hardware.IsValid() == false {
				console.Error("system/" + system.Name + ": Invalid Device \"" + hardware.Name + "\"")
				console.Error(utils_encoding.ToJSON(hardware))
			}
		...
		for i := 0; i < len(system.Incidents); i++ {
			...
			var incident = system.Incidents[i]

			if incident.IsValid() == false {
				console.Error("system/" + system.Name + ": Invalid Incident \"" + incident.Type + "/" + incident.Datetime.String() + "\"")
				console.Error(utils_encoding.ToJSON(incident))
			}
		...
		for n := 0; n < len(system.Networks); n++ {
			...
			var network = system.Networks[n]

			if network.IsValid() == false {
				console.Error("system/" + system.Name + ": Invalid Network \"" + network.Name + "\"")
				console.Error(utils_encoding.ToJSON(network))
			}
		...
		for p := 0; p < len(system.Packages); p++ {
			...
			var pkg = system.Packages[p]

			if pkg.IsValid() == false {
				console.Error("system/" + system.Name + ": Invalid Package \"" + pkg.Name + "\"")
		...
		for r := 0; r < len(system.Responses); r++ {
			...
			var response = system.Responses[r]

			if response.IsValid() == false {
				console.Error("system/" + system.Name + ": Invalid Response \"" + response.Type + "/" + response.Datetime.String() + "\"")
				console.Error(utils_encoding.ToJSON(response))
			}
		...
		for s := 0; s < len(system.Services); s++ {
			...
			var service = system.Services[s]

			if service.IsValid() == false {
				console.Error("system/" + system.Name + ": Invalid Service \"" + service.Name + "\"")
				console.Error(utils_encoding.ToJSON(service))
			}
		...
		for a := 0; a < len(system.Antiques); a++ {
			...
			var antique = system.Antiques[a]

			if antique.IsValid() == false {
				console.Error("system/" + system.Name + ": Invalid Antique \"" + antique.Name + "\"")
				console.Error(utils_encoding.ToJSON(antique))
			}
		...
		for u := 0; u < len(system.Updates); u++ {
			...
			var update = system.Updates[u]

			if update.IsValid() == false {
				console.Error("system/" + system.Name + ": Invalid Update \"" + update.Name + "\"")
				console.Error(utils_encoding.ToJSON(update))
			}
		...
		for u := 0; u < len(system.Users); u++ {
			...
			var user = system.Users[u]

			if user.IsValid() == false {
				console.Error("system/" + system.Name + ": Invalid User \"" + user.Name + "\"")
				console.Error(utils_encoding.ToJSON(user))
			}

		}

		if system.Fingerprint.Country == "" {
			console.Error("system/" + system.Name + ": Invalid Fingerprint Country")
		}

		if system.Fingerprint.Locale == "" {
			console.Error("system/" + system.Name + ": Invalid Fingerprint Locale")
		}

		if system.Fingerprint.Timezone == "" {
			console.Error("system/" + system.Name + ": Invalid Fingerprint Timezone")
		}
